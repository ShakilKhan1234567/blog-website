				...
				<div class="col-lg-8">
                    ...
                    <div class="post post-single">
						...
						<div class="post-header">
							<h1 class="title mt-0 mb-3">{!! $blogs->blog_title !!}</h1>
							<ul class="meta list-inline mb-0">
								<li class="list-inline-item"><a href="#"><img style="width: 50px; height:50px; border-radius:50%" src="{{ asset('uploads/user/') }}/{{ $blogs->rel_to_user->photo }}" class="author" alt="author"/>{{ $blogs->rel_to_user->name }}</a></li>
								<li class="list-inline-item"><a href="#">{{ $blogs->rel_to_category->category_name }}</a></li>
								<li class="list-inline-item">{{ $blogs->created_at->format('d M Y') }}</li>
							</ul>
						</div>
						<!-- featured image -->
						<div class="featured-image">
							<img width="80%" src="{{ asset('uploads/blog/') }}/{{ $blogs->blog_image }}" alt="post-title" />
						</div>
						<!-- post content -->
					...
					<div class="comments bordered padding-30 rounded">
						...
						<ul class="comments">
                            ...
                            <li class="comment rounded">
								<div class="thumb">
									<img style="width: 50px; height:50px; border-radius:50%;" src="{{ Avatar::create($comment->name)->toBase64() }}" alt="John"/>
								</div>
								<div class="details">
									<h4 class="name"><a href="#">{{ $comment->name }}</a></h4>
									<span class="date">{{ $comment->created_at->format('M d, Y') }}</span>
									<p>{{ $comment->comment }}</p>
									<a href="{{ route('comment.reply',$comment->id) }}" class="btn btn-default btn-sm">Reply</a>
								</div>
							</li>
                            @foreach (App\Models\Reply::where('comment_id', $comment->id)->get() as $reply)
                            <li class="comment child rounded">
								<div class="thumb">
									<img style="width: 50px; height:50px; border-radius:50%;" src="{{ Avatar::create($reply->name)->toBase64() }}" />
								</div>
								<div class="details">
									<h4 class="name"><a href="#">{{ $reply->name }}</a></h4>
									<span class="date">{{ $reply->created_at->format('M d, Y') }}</span>
									<p>{{ $reply->reply }}</p>
								</div>
							</li>
                            @endforeach
					...
					<!-- section header -->
					<div id="comment" class="section-header">
					...
					<div class="comment-form rounded bordered padding-30">
						<form id="comment-form" class="comment-form" action="{{ route('comment.store', $blogs->id) }}" method="post">
                            @csrf
